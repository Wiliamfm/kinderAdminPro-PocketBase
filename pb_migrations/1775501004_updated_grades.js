...
migrate((app) => {
  ...
  unmarshal({
    ...
    "listRule": "@request.auth.roles ?= \"enrollment_admin\" || @request.auth.roles ?= \"reports_admin\" || @request.auth.roles ?= \"events_admin\" || @request.auth.is_admin = true",
    "updateRule": "@request.auth.roles ?= \"enrollment_admin\" || @request.auth.is_admin = true",
    "viewRule": "@request.auth.roles ?= \"enrollment_admin\" || @request.auth.roles ?= \"reports_admin\" || @request.auth.roles ?= \"events_admin\" || @request.auth.is_admin = true"
  }, collection)
